//页面tab栏切换
$(".debtManagementTitle").click(function(){
    $(this).addClass('TitleActive').siblings().removeClass('TitleActive');
    $('.mainContWarp').eq($(".debtManagementTitle").index($(this))).css("display","block").siblings().css("display","none")
})
//二级详情页面打开
function openLocation(url,id){
...
(function(){   
    function addressChange(province,city){
        var pHtmlStr = '<option value="">请选择省份</option>'; 
        for(var name in pc){ 
            pHtmlStr = pHtmlStr + '<option value='+name+'>'+name+'</option>';  
        }  
        $('#'+province).html(pHtmlStr);
        $('#'+province).change(function(){
            var pname = $("#"+province).find("option:selected").text();
            var cityList = pc[pname];  
            var pHtmlStr = '<option value="" style="color:#333;">请选择城市</option>';   
            for(var index in cityList){  
                pHtmlStr = pHtmlStr + '<option value='+cityList[index]+' style="color:#333;">'+cityList[index]+'</option>';  
            }  
            $('#'+city).html(pHtmlStr);
            $('#'+city).change();
        })
    ...
    addressChange("localProvince","localCity");
    addressChange("regisProvince","regisCity")
})();